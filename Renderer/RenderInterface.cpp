...
class ResourceContainer {
    ...
    */
    uint32_t ReserveIndex() { return next_id_++; }
    ...
    */
    ...
    friend Pair* begin(ResourceContainer& c) { return c.data_.data(); }

    friend const Pair* begin(const ResourceContainer& c) { return c.data_.data(); }

    friend Pair* end(ResourceContainer& c) { return c.data_.data() + c.data_.size(); }

    friend const Pair* end(const ResourceContainer& c) { return c.data_.data() + c.data_.size(); }
};
        ...
        [=](const PassData& data) {
            for (auto& pair : meshes) {
                ...
                if (mesh.visible) {
                    ...
                    gl::SetState(
                        gl::RenderState::RGB_WRITE | gl::RenderState::ALPHA_WRITE |
                        gl::RenderState::DEPTH_WRITE | gl::RenderState::DEPTH_TEST_LESS |
                        gl::RenderState::PRIMITIVE_TRIANGLES |
                        (mesh.two_sided ? gl::RenderState::CULL_OFF : gl::RenderState::CULL_CCW) |
                        (mesh.draw_wireframe ? gl::RenderState::POLYGON_MODE_LINE
...
void ForwardRendererPrototype()
{
    /*
        ...
        1. Compute shader(s) is dispatched to compute lighting clusters. We have a buffer of lights,
       as just a flat array. This shader runs for each light and computes a cluster list, where each
       element of this cluster list has an offset and count into an item list, which stores actual
       light indices. This may not be as easily parallelized as I thought - computing the counts can
       be easily done using atomics, but offsets needs a prefix scan, and the item list needs to be
       sorted. So probably better done on the CPU. May be easier to just do this computation
       serially.
        2. Optional: Depth pre-pass. Avoids overdraw, so probably worthwhile. Also outputs a
       velocity buffer - probably won't bother with that for a while.
        3. Forward pass: For each mesh, draw it to the frame buffer. Do lighting lookups here, etc.
        For now, this would even be enough for me. I can probably do everything here in a single
       pass, but maybe not IBL. If I need to, I can do that afterwards.

        3. Debug pass: Anything that needs to get drawn without lighting, draw to a separate buffer,
       but doing depth testing against the forward pass. IMGUI stuff can be drawn here as well, to a
       fullscreen framebuffer.
        4. Optional: SSAO pass? / Tonemapping and postprocessing goes here.
        5. Composition: Combing the current color buffer with the debug pass output, then blit to
       screen.
        ...
        so it should be fine to just switch, handle a few types specifically? Maybe to a trace of
       Doom in renderdoc, and see what the buffers/shaders look like.
    */
}
...
void RenderLoop(GLFWwindow* window)
{
    ...
    while (true) {
        ...
        render_commands.SwapBuffers();  // Need to signal to the other threads that they can now
                                        // make render calls.
        postrender_commands.SwapBuffers();
        game_fence.clear();

        // Update our state by pumping the command list. This syncs state between the game + render
        // threads.
...
{
    auto cmd = render_commands.Add([=]() { glViewport(0, 0, w, h); });
}

RenderResource CreateGeometry(
...
void UpdateGeometry(
    const RenderResource geometry_handle,
    const MemoryBlock*   vertex_data,
    const MemoryBlock*   index_data)
{
...
void SetMeshVisibility(const RenderResource mesh, bool visible)
{
    ...
    render_commands.Add([=]() { meshes[mesh].visible = visible; });
}

void SetMeshTwoSided(const RenderResource mesh, bool two_sided)
{
    Expects(GetResourceType(mesh) == ResourceType::MESH);
    render_commands.Add([=]() { meshes[mesh].two_sided = two_sided; });
}

void SetMeshDrawWireframe(const RenderResource mesh, bool wireframe)
{
    Expects(GetResourceType(mesh) == ResourceType::MESH);
    render_commands.Add([=]() { meshes[mesh].draw_wireframe = wireframe; });
}
...
{
    render_commands.Add([=]() { view_matrix = matrix; });
}

void SetProjectionTransform(const Mat4& matrix)
{
    render_commands.Add([=]() { projection_matrix = matrix; });
}

void EndFrame()
{
    render_fence.clear();
    while (game_fence.test_and_set(std::memory_order_acquire)) { std::this_thread::yield(); }
}
...
void InitImguiRendering(const MemoryBlock* font_data, int width, int height)
{
    render_commands.Add([=]() {
        ...
        vert_layout
            ...
            .Add(
                VertexLayout::AttributeBinding::COLOR0,
                4,
                VertexLayout::AttributeType::UINT8,
                true);
...
{
    render_commands.Add([=]() {
        constexpr uint64_t raster_state =
            0 | gl::RenderState::BLEND_EQUATION_ADD | gl::RenderState::BLEND_ONE_MINUS_SRC_ALPHA |
            gl::RenderState::CULL_OFF | gl::RenderState::DEPTH_TEST_OFF;
