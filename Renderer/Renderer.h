...
/*
    ...
    Right now, requires an opengl context to be created externally and current on the thread which
   calls Initialize and Render. All other calls can only be made from the same thread.
...
const MemoryBlock* MakeRef(
    const void*    data,
    const uint32_t size,
    ReleaseFunction = nullptr,
    void* user_data = nullptr);  // Creates a reference to memory which is managed by the user. Must
                                 // be kept for two frames
...
unsigned int GetNumUniforms(
    ProgramHandle h);  // NOTE: This function must be called a frame after the program was created.
...
TextureHandle          CreateTexture2D(
             ...
             const MemoryBlock* data = nullptr);
...
// Sets uniforms - these will be set on the program of whatever the next submit call is on the
// calling thread.
...
void SetVertexBuffer(
    ...
    uint32_t           num_verts    = UINT32_MAX);
void SetIndexBuffer(
    IndexBufferHandle h,
    uint32_t          first_element = 0,
    uint32_t          num_elements  = UINT32_MAX);
