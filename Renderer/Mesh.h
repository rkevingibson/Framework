...
class Mesh {
   public:
    ...
    inline Vec3* Positions() { return GetVec3Attribute(MeshAttributes::POSITION); }

    inline const Vec3* Positions() const { return GetVec3Attribute(MeshAttributes::POSITION); }

    inline Vec3* Normals() { return GetVec3Attribute(MeshAttributes::NORMAL); }

    inline const Vec3* Normals() const { return GetVec3Attribute(MeshAttributes::NORMAL); }

    inline bool HasColors() const { return (active_attributes_ & MeshAttributes::COLOR0) != 0; }

    inline Vec4* Colors() { return GetVec4Attribute(MeshAttributes::COLOR0); }

    inline const Vec4* Colors() const { return GetVec4Attribute(MeshAttributes::COLOR0); }

    inline unsigned int* Indices() { return static_cast<unsigned int*>(index_block_.ptr); }
    ...
    */
    ...
    {
        return Eigen::Map<Eigen::Matrix<float, 3, Eigen::Dynamic>>(
            (float*)vertex_block_.ptr, 3, num_verts_);
    }
    ...
    */
    ...
    {
        return Eigen::Map<Eigen::Matrix<unsigned int, 3, Eigen::Dynamic>>(
            (unsigned int*)index_block_.ptr, 3, num_indices_ / 3);
    }
    void ComputeNormals();
    ...
    {
        Vec3 centroid(0, 0, 0);
        for (int i = 0; i < num_verts_; i++) { centroid += Positions()[i]; }
        centroid /= num_verts_;
        return centroid;
    }
    ...
    inline int VertexSize() const { return vertex_size_; }
   ...
   private:
    ...
    inline Vec3* GetVec3Attribute(MeshAttributes::Enum attr)
    {
        ...
        return reinterpret_cast<Vec3*>(
            static_cast<float*>(vertex_block_.ptr) + attribute_offset_[index]);
    }
    ...
    /*
        ...
        Useful for adding per-face colors. This has the effect of no longer needing an index list,
       really - the indices are just 1..n
    */
    ...
    */
    ...
    // TODO: Fix this crappy attribute code. Let us set how large each attribute is, and treat as a
    // byte array. Use template functions to get attributes if I don't feel like dealing with floats.
    // E.g. GetAttribute<Vec3>(MeshAttribute::Position); Can optionally specialize on the standard
    // ones, asserting to enforce convention.
};
