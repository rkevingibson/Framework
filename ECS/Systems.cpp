...
void KeyCallback(GLFWwindow* win, int key, int scancode, int action, int mods)
{
    ...
    if (!ImGui::GetIO().WantCaptureKeyboard) {
        ...
        Input::SetKeyStatus(
            static_cast<Input::Keyname>(key), static_cast<Input::KeyAction>(action));
    }
}
...
void MouseButtonCallback(GLFWwindow* win, int button, int action, int mods)
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse) {
        ...
        // Probably a bad idea to assume that won't change, but it seems reasonable. Left = 0, Right
        // = 1, Middle = 2. Why would that change?
...
void ScrollCallback(GLFWwindow* win, double xoffset, double yoffset)
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse) {
        rkg::Input::MouseWheelDelta =
            Vec2{(float)xoffset,
                 (float)yoffset};  // Usually we care about yoffset, maybe I should put it first?
    }
}
...
GLFWwindow* InitializeGLFW()
{
    ...
    GLFWwindow* window = glfwCreateWindow(
        Input::ScreenSize.x, Input::ScreenSize.y, "Material Editor", nullptr, nullptr);
...
void Run()
{
    ...
    while (!glfwWindowShouldClose(window) && running) {
        ...
        while (accumulator >= fixed_timestep) {
            for (auto sys : systems) { sys->FixedUpdate(); }
            accumulator -= fixed_timestep;
        }

        for (auto sys : systems) { sys->Update(frame_time); }
